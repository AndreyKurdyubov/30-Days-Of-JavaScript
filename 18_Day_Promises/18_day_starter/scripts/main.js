...
fetch(countriesAPI)
   ...
   .then(data => {
      console.log('data[0] :>> ', data[0]);
      data.forEach(el => {
         topCountries.push({name: el.name, population: el.population})
         el.languages.forEach(lang => {
            languages.add(lang.name)
         })
      });
      topCountries.sort((a, b) => b.population - a.population);
      console.log('topCountries :>> ', topCountries.slice(0, 10));
      console.log('languages :>> ', languages, languages.size);
   })
   .catch(err => {
      console.log('err :>> ', err);
   });